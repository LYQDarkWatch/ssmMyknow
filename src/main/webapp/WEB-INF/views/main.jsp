...
<div class="jumbotron">
    ...
    <p><a class="btn btn-primary btn-lg" href="/mystudy_war_exploded/knowledge/knowledgedet?kid=23" role="button">查看内容</a></p>
</div>
...
<div>
    <div id="type" style="width: 100px;float: left;border-right: 1px solid black;border-bottom: 1px solid black;text-align:center;margin-top: 60px;margin-left: 500px;background-color: white">
    </div>
<div id="test" style="width: 800px;margin-left: 50px;float: left;background-color: white ; margin-top: 60px" >
</div>
</div>
</body>
    ...
    window.onload = function () {
        $.ajax({
            ...
            success: function (data) {
                var html = '';
                var html1='';
                data.knowledgeList.map(function (item, index) {
                        html +=
                            ' <div style="width: 200px;height: 350px;float: left;" class="divt">'
                            +'<strong >知识点</strong>：'+'<br>'+
                            '<a href="/mystudy_war_exploded/knowledge/knowledgedet?kid=' + item.kid + '">' +
                            item.kname +
                            '</a>' +'<br>'+'<strong>知识点内容</strong>'+'<br>'+
                             '<p class="font">'+item.pnitro+'</p>' +
                            '<br>'+'<strong style="position:relative">上传人</strong>'+'<br>'+
                            item.username +
                            '<br>'+'<strong>知识点类型</strong>'+'<br>'+item.typename
                            +'</div>'

                        html1+=
                    '<p style="font-size:20px">'+'<a href="/mystudy_war_exploded/knowledge/typeknowledge?typename=' + item.typename + '">'+item.typename+'</a>'+'</p>'
                });
                $('#test').append(html);
                $('#type').append(html1);
        ...
        window.onload = function () {
        $.ajax({
            url: '/mystudy_war_exploded/knowledge/gettype',
            type: 'get',
            dataType: 'json',
            success: function (data1) {
                var html2='';
                data1.knowledgetype.map(function (type, index1) {
                    html2+=
                        '<p style="font-size:20px">'+'<a href="/mystudy_war_exploded/knowledge/typeknowledge?typename=' + type.typename + '">'+type.typename+'</a>'+'</p>'
                });
                $('#type').append(html2);
